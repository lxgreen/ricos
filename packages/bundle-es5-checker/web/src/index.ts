/* eslint-disable @typescript-eslint/no-unused-vars */
import { RicosEditor } from 'ricos-editor';
import { RicosViewer } from 'ricos-viewer';
import { pluginCodeBlock, createCodeBlockPlugin } from 'wix-rich-content-plugin-code-block';
import { pluginDivider, createDividerPlugin } from 'wix-rich-content-plugin-divider';
import { pluginEmoji, createEmojiPlugin } from 'wix-rich-content-plugin-emoji';
import { pluginFileUpload, createFileUploadPlugin } from 'wix-rich-content-plugin-file-upload';
import { pluginGallery } from 'wix-rich-content-plugin-gallery';
import { pluginGiphy, createGiphyPlugin } from 'wix-rich-content-plugin-giphy';
import { pluginHashtag } from 'wix-rich-content-plugin-hashtag';
import { pluginHeadersMarkdown } from 'wix-rich-content-plugin-headers-markdown';
import { pluginHtml, htmlButtonsTypes } from 'wix-rich-content-plugin-html';
import { pluginImage, createImagePlugin } from 'wix-rich-content-plugin-image';
import { pluginHeadings } from 'wix-rich-content-plugin-headings';
import { pluginLineSpacing, createLineSpacingPlugin } from 'wix-rich-content-plugin-line-spacing';
import { pluginLink, createLinkPlugin } from 'wix-rich-content-plugin-link';
import { pluginLinkButton, pluginActionButton } from 'wix-rich-content-plugin-button';
import { pluginLinkPreview, LinkPreviewProviders } from 'wix-rich-content-plugin-link-preview';
import { pluginMap } from 'wix-rich-content-plugin-map';
import { pluginPoll } from 'wix-rich-content-plugin-social-polls';
import { pluginSpoiler } from 'wix-rich-content-plugin-spoiler';
import { pluginCollapsibleList } from 'wix-rich-content-plugin-collapsible-list';
import { pluginMentions } from 'wix-rich-content-plugin-mentions';
import { pluginUndoRedo } from 'wix-rich-content-plugin-undo-redo';
import { pluginVideo, createVideoPlugin, videoButtonsTypes } from 'wix-rich-content-plugin-video';
import { pluginTable } from 'wix-rich-content-plugin-table';
import './loadable';
// eslint-disable-next-line no-console
console.log(
  'packages',
  RicosViewer,
  RicosEditor,
  pluginCodeBlock,
  pluginDivider,
  pluginEmoji,
  pluginFileUpload,
  pluginGallery,
  pluginGiphy,
  pluginHashtag,
  pluginHeadersMarkdown,
  pluginHtml,
  pluginVideo,
  pluginTable
);
