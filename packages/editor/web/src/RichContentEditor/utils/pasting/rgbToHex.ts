...
const tooDark = (luma: number) => luma <= 50;

const tooLight = (luma: number) => luma > 220;

const shouldConvertRGB = (rgb: string[] | null) => {
  if (!rgb) {
    return false;
  }
...
export default (rgbString: string) => {
  const rgb = parseRGB(rgbString);

  if (shouldConvertRGB(rgb) && rgb) {
