...
const normalizeSafariSpaceSpans = (htmlString: string) => {
...
const normalizeSpacing = (htmlString: string) => {
...
const normalizeSpacerunSpans = (htmlDocument: Document) => {
  htmlDocument.querySelectorAll('span[style*=spacerun]').forEach((el: HTMLElement) => {
...
const replaceSpansWithOnlySpaces = (html: string) =>
  // eslint-disable-next-line no-irregular-whitespace
  html.replace(/(<span> <\/span>)/g, ' ');

const cleanContentAfterBody = (htmlString: string) => {
...
export default function normalizeHTML(htmlString?: string) {
  if (!htmlString) {
    return undefined;
  }
