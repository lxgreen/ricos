...
const draftContentWithInvalidImages = {
  blocks: [
    {
      key: '50k2j',
      text: 'image:',
...
const draftContentWithInvalidVideos = {
  blocks: [
    {
      key: 'foo',
      text: '',
...
describe('extractMedia (DraftContent)', () => {
  ...
  it('should not crash on invalid video data', () => {
    const expected = [
      {
        // video
        videoThumbnailUrl: '',
      ...
      {
        // gallery
        // no metadata in content -- item treated as image
        imageUrl: '',
    ...
    const actual = extractMedia(draftContentWithInvalidVideos);
    expect(actual).toStrictEqual(expected);
  });
  ...
  it('should not crash on invalid image data', () => {
    const expected = [
      {
        imageUrl: '',
        imageWidth: undefined,
    ...
    const actual = extractMedia(draftContentWithInvalidImages);
    expect(actual).toStrictEqual(expected);
  });
});
