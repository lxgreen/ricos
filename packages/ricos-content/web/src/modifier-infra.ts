...
import { fromTraversable, Prism, Traversal } from 'monocle-ts';
 ...
 */
export const getModifier = <NodeT>(
  ...
  const modifyById = (idsToSet: string[], setter: (node: NodeT) => NodeT | NodeT[]) => (
  ...
  const foldTree = (tree: T.Tree<ModTreeNode<NodeT>>, setter: (node: NodeT) => NodeT | NodeT[]) => (
    idsToSet: string[]
  ): NodeT =>
